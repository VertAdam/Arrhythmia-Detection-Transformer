...
# 15 second model
class CTN_SingleBeat(nn.Module):
    def __init__(self, embedding_size, nhead, feed_fwd_layer_size, num_layers, dropout, fc1_size, wide_feature_size, classes, num_leads = 1):
        super(CTN_SingleBeat, self).__init__()
        #TODO: CHANGE THE CONV PARAMETERS SO THAT IT FITS A 187 SIZE INPUT
        self.encoder = nn.Sequential(
            nn.Conv1d(num_leads, 128, kernel_size=5, stride=3, padding=0, bias=False),
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),
            nn.Conv1d(128, 256, kernel_size=5, stride=2, padding=0, bias=False),
            nn.BatchNorm1d(256),
            nn.ReLU(inplace=True),
            nn.Conv1d(256, embedding_size, kernel_size=2, stride=1, padding=0, bias=False),
            nn.BatchNorm1d(embedding_size),
            nn.ReLU(inplace=True),
            nn.Conv1d(embedding_size, embedding_size, kernel_size=2, stride=1, padding=0, bias=False),
            nn.BatchNorm1d(embedding_size),
            nn.ReLU(inplace=True),
            nn.Conv1d(embedding_size, embedding_size, kernel_size=2, stride=1, padding=0, bias=False),
            nn.BatchNorm1d(embedding_size),
